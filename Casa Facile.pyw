...
GITHUB_FILE_URL = "https://raw.githubusercontent.com/Renato-4132/Gestione-spese/refs/heads/main/casa facile.pyw"

NOME_FILE = "casa facile.py"  # Nome del file da salvare
...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Casa Facile pro v.3.9.0")
    ...
    def show_info_app(self):
            ...
            info = (
                ...
                "• Per esportare i dati di un mese o di un anno specifico, seleziona mese e anno dal riquadro sinistro e poi usa i pulsanti 'Esporta mese da estratto' o 'Esporta anno da estratto'.\n\n"
                 ...
                 "• I plugin python sono autoinstallanti, ma per buona promemoria, allego come installare i pacchetti aggiuntivi \n"
    ...
    def utenze(self):
        ...
        def esporta_preview():
            ...
            for utenza in utenze:
                ...
                totale_consumi = 0.0  # Variabile per il totale consumi
                for iid in self.trees[utenza].get_children():
                    ...
                    totale_consumi += float(cons)  # Accumula il totale dei consumi
                txt.insert(tk.END, "-" * 45 + "\n")
                txt.insert(tk.END, f"{'Totale':10s} {'':>10s} {'':>10s} {totale_consumi:10.2f}\n\n")
        ...
        def salva_dati_letture(letture_salvate):
            """Salva i dati in un file scelto dall'utente in formato testo, includendo i totali annuali."""
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    for utenza, anni in letture_salvate.items():
                        ...
                        for anno, dati in sorted(anni.items(), reverse=True):
                            f.write(f"  {anno}:\n")
                            tot_annuo = 0.0  # Variabile per accumulare il totale annuale
                            for r in dati:
                                f.write(f"    {r[0]} - {r[3]} {utenza}\n")
                                tot_annuo += float(r[3])  # Somma i consumi mensili
                            f.write(f"  Totale {anno}: {tot_annuo:.2f} {utenza}\n\n")  # Scrive il totale annuale
